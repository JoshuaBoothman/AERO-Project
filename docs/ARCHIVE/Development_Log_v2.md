...
## [2026-01-11] - Global Azure Authentication Patch
**Milestone:** Resolved "Unauthorized" errors across the entire platform on Azure Production

### Completed Items
*   **Authentication (Backend)**
    *   **Reason:** Azure's "EasyAuth" middleware was intercepting and overwriting the standard `Authorization: Bearer <token>` header, causing JWT verification failure (`JsonWebTokenError: invalid signature`) in the Node.js function app.
    *   **Fix:** Updated `lib/auth.js` to prioritize checking a custom `X-Auth-Token` header before falling back to the standard Authorization header. This allows our valid JWT to bypass Azure's interference.
    *   **Security:** Reverted temporary hardcoded secrets back to `process.env.JWT_SECRET` now that the header transport issue is resolved.
*   **Frontend (Client)**
    *   **Patching:** Systematically updated **ALL** protected `fetch()` calls to include the `X-Auth-Token` header alongside the standard Authorization header.
    *   **Scope:** Applied this fix to:
        *   **Admin Core:** `AdminDashboard`, `AdminList` (Manage Admins), `OrgSettings` (Uploads/Updates).
        *   **Camping:** `AdminMapTool` (Bulk Add, Move, Rename, Delete).
        *   **Commerce:** `ProductEditor` (SKUs, Options, Images), `MerchandiseList`.
        *   **Events:** `EventForm` (Venue, Images), `AdminSubevents`.
        *   **Assets:** `AssetTypes`, `AssetItems`, `AssetHires`.
        *   **User:** `MyOrders` (History), `Checkout` (Payment Processing).
*   **Verification**
    *   Confirmed fix allows Admin actions (e.g., updating settings, managing users) that were previously returning 403 Forbidden.

### Next Steps
*   **Monitoring:** Monitor Azure logs for any edge cases.
